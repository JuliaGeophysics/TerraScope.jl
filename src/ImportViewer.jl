...
function _choose_import_file!(textbox; start_dir=pwd())
    ...
    function list_directory(path)
        try
            ...
            choices = Pair{String,String}["Choose…"=>"", "../"=>dirname(folder)]
...
function _open_import_dialog(session, on_loaded; start_dir=pwd(), display_window=true)
    ...
    Label(fig[5,1:3],"Source CRS is required, except when read from a .prj/raster or a ModEM local origin.",fontsize=12)
    ...
    status = Observable("Z uses metres in a shared vertical datum. Horizontal CRS conversion does not change that datum.")
    ...
    on(load_button.clicks) do _
        ...
        @async try
            ...
            status[] = "Imported $(layer.name). Choose another file to add another layer."
        catch err
...
function launch_viewer(; open_fullscreen=false, block=!isinteractive(), data_root=pwd(), display_window=true)
    session = ImportSession()
    fig = Figure(size=(1550,960))
    toolbar = fig[1,1] = GridLayout()
    Label(toolbar[1,1],"1. Project CRS")
    presets = Menu(toolbar[1,2],options=["Choose a coordinate system…"=>"",
        "GDA94 / MGA 54 (Cloncurry)"=>"EPSG:28354", "GDA2020 / MGA 54"=>"EPSG:7854",
        "WGS84 / UTM 54S"=>"EPSG:32754","ETRS89 / TM35FIN"=>"EPSG:3067"])
    crs_box = Textbox(toolbar[1,3],stored_string=" ",placeholder="EPSG code or PROJ string",width=340)
    set_crs = Button(toolbar[1,4],label="Use CRS")
    import_button = Button(toolbar[1,5],label="2. Import…")
    status = Observable("Choose a project coordinate system before importing any data.")
    ax = Axis3(fig[2,1],xlabel="Easting (m)",ylabel="Northing (m)",zlabel="Elevation (m)",aspect=:data)
    controls = fig[3,1] = GridLayout()
    ...
    Label(fig[4,1],status,tellwidth=false,halign=:left)
    colorbar = Colorbar(fig[2,2],limits=(0,1),colormap=:viridis,label="")
    ...
    function select_layer(index)
        ...
        colorbar.limits[] = layer.colorrange
